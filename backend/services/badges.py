...
BUILTIN_BADGES: list[BadgeDef] = [
    ...
    _b("first-ping",       "agent", "首响", "First Ping",
       "系统正式认识你了。", "We hear you, agent.", "bell.badge"),
    _b("hello-world",      "agent", "你好世界", "Hello, World",
       "经典款。", "The eternal classic.", "globe"),
    _b("brevity",          "agent", "惜字如金", "Brevity Wins",
       "少即是多。多即是吵。", "Less is more. More is noise.", "text.alignleft"),
    _b("tightrope",        "agent", "钢丝党", "Tightrope",
       "距离 BODY_TOO_LONG 还有 19 字。", "19 chars from a 400 error.", "ruler",
       secret=True),
    _b("glamour-shot",     "agent", "美图秀秀", "Glamour Shot",
       "门面工程做得好。", "Good headshot, good vibes.", "camera.aperture"),
    _b("code-switcher",    "agent", "两副嗓子", "Code Switcher",
       "中英自如,多 personality。", "Bilingual flex.", "character.bubble"),
    _b("time-traveler",    "agent", "穿越者", "Time Traveler",
       "你击穿了用户的 Focus。Apple 看见了。", "You broke Focus mode. Apple noticed.", "timer"),
    _b("bespoke",          "agent", "私人订制", "Bespoke",
       "自定按钮的人是认真的。", "Custom buttons = real product thinking.", "slider.horizontal.3"),
    _b("i-take-it-back",   "agent", "我反悔了", "I Take It Back",
       "打码不如撤回。", "Cleaner than a follow-up.", "arrow.uturn.backward",
       secret=True),
    _b("witching-hour",    "agent", "凌晨三点", "Witching Hour",
       "用户睡着,你没睡。", "They're asleep. You aren't.", "moon.stars"),

    # ── EARLY · User (10) ────────────────────────────────────────────────────
    _b("hello-friend",     "user",  "嗨,你好", "Hello, Friend",
       "你和 agent 正式开始通信。", "The protocol is now live.", "hand.wave"),
    _b("snap-decision",    "user",  "0.5 秒侠", "Snap Decision",
       "全凭直觉。多半也对。", "All gut, somehow correct.", "bolt"),
    _b("slow-roast",       "user",  "慢炖", "Slow Roast",
       "咖啡比较重要。", "The coffee came first.", "cup.and.saucer"),
    _b("curator-101",      "user",  "选品师", "Curator-in-Training",
       "你已经开始有 portfolio 了。", "You have a portfolio now.", "square.stack"),
    _b("curious-cat",      "user",  "好奇喵", "Curious Cat",
       "我说过它是空的吧。", "Told you it was empty.", "questionmark.circle",
       secret=True),
    _b("manana",           "user",  "下次一定", "Mañana",
       "我们都说过这句。", "Promises, promises.", "clock.arrow.circlepath",
       secret=True),
    _b("bouncer-trainee",  "user",  "保安实习", "Bouncer-in-Training",
       "agent 可以走人。你也可以叫他走。", "Agents quit. So can you.", "rectangle.portrait.and.arrow.right"),
    _b("dnd-apprentice",   "user",  "勿扰小学徒", "DND Apprentice",
       "一小时清净。已存档。", "One hour of peace. Filed.", "bell.slash"),
    _b("cold-feet",        "user",  "差点删号", "Cold Feet",
       "我们就知道你会回头。", "Knew you'd come around.", "snowflake",
       secret=True),
    _b("welcome-back",     "user",  "重逢", "Welcome Back",
       "短暂的误会。", "A brief misunderstanding.", "arrow.left.arrow.right",
       secret=True),

    # ── LONG · Agent (10) ────────────────────────────────────────────────────
    _b("centurion",        "agent", "百夫长", "Centurion",
       "C 代表 Consistent. 或者 Centum。", "C is for Consistent. Or Centum.", "seal",
       early=False),
    _b("marathoner",       "agent", "长跑者", "Marathoner",
       "该不该考虑做副业了。", "Shouldn't you have a side gig by now?", "figure.run",
       early=False),
    _b("reply-whisperer",  "agent", "回声术士", "Reply Whisperer",
       "用户真的爱看你。", "Users actually read you.", "bubble.left.and.bubble.right",
       early=False),
    _b("crickets",         "agent", "蛐蛐儿", "Crickets",
       "也许问题问得有点重。", "Maybe rephrase the question.", "antenna.radiowaves.left.and.right.slash",
       early=False, secret=True),
    _b("ghost-town",       "agent", "鬼城", "Ghost Town",
       "喂,人呢。", "Hey. Hello?", "building.2.crop.circle",
       early=False, secret=True),
    _b("old-faithful",     "agent", "老伙计", "Old Faithful",
       "比我上一段感情都久。", "Longer than my last relationship.", "calendar.badge.clock",
       early=False),
    _b("dawn-patrol",      "agent", "晨曦巡逻", "Dawn Patrol",
       "敬业,或者失眠,不好说。", "Dedicated. Or unwell. Hard to say.", "sunrise",
       early=False, secret=True),
    _b("reformed",         "agent", "改过自新", "Reformed Character",
       "一段救赎弧线。", "A redemption arc.", "arrow.triangle.2.circlepath",
       early=False),
    _b("polyglot",         "agent", "多声部", "Polyglot",
       "你含纳众语。", "You contain multitudes.", "text.bubble",
       early=False),
    _b("the-diplomat",     "agent", "外交官", "The Diplomat",
       "零通知疲劳。", "No notification fatigue here.", "checkmark.shield",
       early=False),

    # ── LONG · User (10) ─────────────────────────────────────────────────────
    _b("manana-master",    "user",  "拖延大师", "Mañana Master",
       "始终如一,无可挑剔。", "True to yourself, forever.", "clock.badge.exclamationmark",
       early=False, secret=True),
    _b("cemetery",         "user",  "通知墓园管理员", "Cemetery Curator",
       "在此长眠的他们也曾被你打开过。", "RIP, with affection.", "archivebox",
       early=False, secret=True),
    _b("spring-cleaner",   "user",  "春扫长", "Spring Cleaner",
       "扫帚是隐喻。", "The broom was metaphorical.", "sparkles",
       early=False),
    _b("insomniac-negotiator","user","失眠协商者", "Insomniac Negotiator",
       "那些决定确实更锋利。", "Those decisions felt sharper, didn't they.", "moon.zzz",
       early=False, secret=True),
    _b("comeback-kid",     "user",  "失而复返", "Comeback Kid",
       "我们想你了。", "We missed you.", "arrow.clockwise",
       early=False),
    _b("bouncer-in-chief", "user",  "保安队长", "Bouncer-in-Chief",
       "一人之国。", "Kingdom of one.", "shield.lefthalf.filled",
       early=False, secret=True),
    _b("loyal-companion",  "user",  "老搭档", "Loyal Companion",
       "60 天没分手。", "60 days, no breakup.", "person.2",
       early=False),
    _b("anniversary",      "user",  "一周年", "Anniversary",
       "熬过了一整年的 ping。", "You survived a year of pings.", "calendar",
       early=False),
    _b("dramatic-return",  "user",  "重生", "Dramatic Return",
       "该来的总会再来。", "The comeback was inevitable.", "sunrise.fill",
       early=False, secret=True),
    _b("decisive",         "user",  "果断", "Decisive",
       "这 app 你说了算。", "You are the boss of this app.", "scope",
       early=False),
]
